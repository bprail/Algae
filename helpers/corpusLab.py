...
class Corpus:
    ...
    def getStudentPath(self, student, assignment):
        path = self.config.corpusPath + assignment + '/' + student
        if os.path.exists(path):
            return path
        else:
            return None
            
    def makeStudentPath(self, student, assignment):
        path = self.config.corpusPath + assignment + '/' + student
        if os.path.exists(path):
            return path
        else:
    ...
    def getAssignmentPath(self, student, assignment, filename):
        #path = self.config.corpusPath + assignment + '/' + student + '/' + filename
        pre_path = self.getStudentPath(student, assignment)
        if (pre_path is None):
            return None
    ...
    def cleanPreprocessed(self, student, assignment):
        path = self.config.corpusPath + assignment + '/' + student + '/__algae__'
        if os.path.exists(path):
            shutil.rmtree(path)
